...
namespace Mir.DiscordExtension
{
    public class DiscordsApp
    {
        ...
        public void UpdateActivity()
        {
            try
            {
                _activity.Details = string.Format(
                    _settings.DetailsFormatting,
                    !string.IsNullOrEmpty(_currentState.PlayersName) && _settings.DisplayCharacterName
                        ? _currentState.PlayersName + " "
                        : "",
                    _currentState.PlayerLevel != "0"
                        ? $"Level: {_currentState.PlayerLevel} "
                        : "",
                    _currentState.PlayerCount != -1
                        ? $"UC:{_currentState.PlayerCount} ({_currentState.ObserverCount})"
                        : ""
                );
                SetState();
